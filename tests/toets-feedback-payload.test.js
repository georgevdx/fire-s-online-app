...
assert.ok(
  /Version 1\.3\.113-toets/.test(html) &&
    /appVersion: staging \? '1\.3\.113-toets'/.test(env) &&
    /app\.js\?v=1-3-113-temp-pw/.test(html) &&
    /fireS\.toetsCacheDrop\.1-3-114/.test(html),
  'Toets must cache-bust the Fire-S Test feedback payload fix as 1.3.113-toets'
);
...
assert.strictEqual(row.app_version, '1.3.113-toets');
