...
assert.ok(/1\.3\.44-toets/.test(env), 'Toets-blad version must stay on 1.3.44-toets');
assert.ok(
  /appVersion: staging \? '1\.3\.27-toets' : '1\.3\.39'/.test(liveEnv) &&
    /bounceLegacyToetsQuery/.test(liveEnv),
  'Live Fire-S must be 1.3.39 and keep the old toets-link bounce'
);
assert.ok(
  !/1\.3\.23/.test(liveEnv),
