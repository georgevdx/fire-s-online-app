...
assert.ok(/fire-s-payfast\.js\?v=1-15-buyer/.test(liveHtml));
assert.ok(/fire-s-entitlement\.js\?v=1-3-107-lock/.test(liveHtml));
assert.ok(/service-worker\.js\?v=108-77-live-buyer/.test(liveHtml));
...
assert.ok(/mode\)\.toLowerCase\(\) === 'live'\) return false/.test(livePayfast), 'Live must not treat the owner email as the merchant');
assert.ok(/function liveCheckoutHtml/.test(livePayfast));
