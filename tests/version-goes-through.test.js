...
assert.ok(
  /function fireSDropStuckLiveCache\(/.test(liveHtml) &&
    /fireS\.liveCacheDrop\.1-3-home-lookup/.test(liveHtml) &&
  ...
  'Live must drop the stuck service worker even while the displayed version stays 1.3.65'
);
assert.ok(
...
assert.ok(
  /function fireSDropStuckToetsCache\(/.test(stagingHtml) &&
    /fireS\.toetsCacheDrop\.1-3-109/.test(stagingHtml) &&
...
assert.ok(
  ...
  'Displayed versions stay 1.3.65 live and 1.3.109-toets so a phone can tell it has this build'
);
