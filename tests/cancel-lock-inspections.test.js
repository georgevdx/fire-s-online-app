...
assert.ok(/fire-s-entitlement\.css\?v=1-3-100-pay/.test(stagingHtml));
assert.ok(/#fireSOwnerLists/.test(stagingCss));
assert.ok(/fire-s-home-lock-panel/.test(stagingCss));
assert.ok(/app\.js\?v=1-3-100-pay/.test(stagingHtml));
assert.ok(/Version 1\.3\.100-toets/.test(stagingHtml));
