...
assert.ok(/1\.3\.64/.test(liveEnv), 'Live Fire-S must be 1.3.64');
assert.ok(/1\.3\.75-toets/.test(stagingEnv), 'Toets-blad version must be 1.3.75-toets');
assert.ok(
  /app\.js\?v=1-3-64-route/.test(liveHtml) &&
    /fire-s-env\.js\?v=1-3-64-ver/.test(liveHtml) &&
    /app\.js\?v=1-3-75-toets-schedule/.test(stagingHtml) &&
    /fire-s-env\.js\?v=1-3-75-ver/.test(stagingHtml),
