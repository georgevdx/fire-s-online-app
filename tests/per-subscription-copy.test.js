...
assert.ok(/1\.3\.112-toets/.test(env), 'Toets-blad version must stay on 1.3.112-toets');
assert.ok(
...
assert.ok(
  /R250 per month per login/.test(liveCatalog) &&
    /per month per login is <strong>R250<\/strong>/.test(liveHtml) &&
    !/R349 per email/.test(liveCatalog) &&
    !/R349 per email/.test(liveHtml),
  'Live root must say R250 per month per login'
);
assert.ok(
