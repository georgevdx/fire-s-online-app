...
assert.ok(/1\.3\.78-toets/.test(env), 'Toets-blad version must be 1.3.78-toets');
assert.ok(
...
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.78-toets</.test(html) &&
    /id="cloudVersion">1\.3\.78-toets</.test(html) &&
    /getElementById\('cloudVersion'\)/.test(env),
  'Toets-blad HTML must show 1.3.78-toets before scripts run'
);
assert.ok(
