...
assert.ok(/1\.3\.68-toets/.test(env), 'Toets-blad version must be 1.3.68-toets');
assert.ok(
  /appVersion: staging \? '1\.3\.27-toets' : '1\.3\.60'/.test(liveEnv) &&
    /bounceLegacyToetsQuery/.test(liveEnv) &&
    /function paintVersion\(/.test(liveEnv),
  'Live Fire-S must be 1.3.60, bounce old toets links, and paint the version'
);
...
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.68-toets</.test(html) &&
    /id="cloudVersion">1\.3\.68-toets</.test(html) &&
    /getElementById\('cloudVersion'\)/.test(env),
  'Toets-blad HTML must show 1.3.68-toets before scripts run'
);
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.60</.test(read('index.html')) &&
    /id="cloudVersion">1\.3\.60</.test(read('index.html')),
  'Live HTML must show 1.3.60 before scripts run'
);
assert.ok(
