...
assert.ok(/1\.3\.113-toets/.test(env), 'Toets-blad version must be 1.3.113-toets');
assert.ok(
  /appVersion: staging \? '1\.3\.27-toets' : '1\.3\.66'/.test(liveEnv) &&
    /bounceLegacyToetsQuery/.test(liveEnv) &&
    /function paintVersion\(/.test(liveEnv),
  'Live Fire-S must be 1.3.66, bounce old toets links, and paint the version'
);
...
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.113-toets</.test(html) &&
    /id="cloudVersion">1\.3\.113-toets</.test(html) &&
    /getElementById\('cloudVersion'\)/.test(env) &&
    /function fireSDropStuckToetsCache\(/.test(html) &&
    /fireS\.toetsCacheDrop\.1-3-114/.test(html) &&
    /TOETS-BLAD 1\.3\.113-toets/.test(env),
  'Toets-blad HTML must show 1.3.113-toets before scripts run'
);
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.66</.test(read('index.html')) &&
    /id="cloudVersion">1\.3\.66</.test(read('index.html')),
  'Live HTML must show 1.3.66 before scripts run'
);
assert.ok(
