...
assert.ok(/1\.3\.83-toets/.test(env), 'Toets-blad version must be 1.3.83-toets');
assert.ok(
...
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.83-toets</.test(html) &&
    /id="cloudVersion">1\.3\.83-toets</.test(html) &&
    /getElementById\('cloudVersion'\)/.test(env) &&
    /function fireSDropStuckToetsCache\(/.test(html) &&
    /fireS\.toetsCacheDrop\.1-3-82/.test(html) &&
    /TOETS-BLAD 1\.3\.83-toets/.test(env),
  'Toets-blad HTML must show 1.3.83-toets before scripts run'
);
assert.ok(
