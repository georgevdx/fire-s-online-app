...
assert.ok(/1\.3\.102-toets/.test(env), 'Toets-blad version must be 1.3.102-toets');
assert.ok(
  /appVersion: staging \? '1\.3\.27-toets' : '1\.3\.65'/.test(liveEnv) &&
    /bounceLegacyToetsQuery/.test(liveEnv) &&
    /function paintVersion\(/.test(liveEnv),
  'Live Fire-S must be 1.3.65, bounce old toets links, and paint the version'
);
...
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.102-toets</.test(html) &&
    /id="cloudVersion">1\.3\.102-toets</.test(html) &&
    /getElementById\('cloudVersion'\)/.test(env) &&
    /function fireSDropStuckToetsCache\(/.test(html) &&
    /fireS\.toetsCacheDrop\.1-3-102/.test(html) &&
    /TOETS-BLAD 1\.3\.102-toets/.test(env),
  'Toets-blad HTML must show 1.3.102-toets before scripts run'
);
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.65</.test(read('index.html')) &&
    /id="cloudVersion">1\.3\.65</.test(read('index.html')),
  'Live HTML must show 1.3.65 before scripts run'
);
assert.ok(
