...
assert.ok(
  /Version 1\.3\.104-toets/.test(stagingHtml) &&
    ...
    /1\.3\.104-toets/.test(stagingEnv) &&
    /fire-s-108-70-toets-104/.test(stagingSw),
  'Toets must show 1.3.104-toets so a phone can tell it has the one company building count'
);
assert.ok(
  /function fireSPremisesBuildingKey\(project\)/.test(stagingApp) &&
    ...
    /fireS\.toetsCacheDrop\.1-3-104/.test(stagingHtml),
