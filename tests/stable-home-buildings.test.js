...
assert.ok(
  /Version 1\.3\.91-toets/.test(stagingHtml) &&
    ...
    /fire-s-owner-lists\.js\?v=1-4-home-lock/.test(stagingHtml) &&
    /1\.3\.91-toets/.test(stagingEnv) &&
    /fire-s-108-70-toets-91/.test(stagingSw),
  'Toets must show 1.3.91-toets so a phone can tell it has the one company building count'
);
assert.ok(
  /function fireSPremisesBuildingKey\(project\)/.test(stagingApp) &&
    ...
    /fireS\.toetsCacheDrop\.1-3-91/.test(stagingHtml),
