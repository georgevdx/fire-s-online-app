...
assert.ok(
  ...
  'Live Fire-S must be 1.3.60 after sit dit live'
);
assert.ok(
  /app\.js\?v=1-3-68-toets-home/.test(stagingHtml) &&
    /fire-s-env\.js\?v=1-3-68-ver/.test(stagingHtml) &&
    /fire-s-service-requests\.js\?v=1-2-archive/.test(stagingHtml),
  'Toets-blad must cache-bust the archive follow-up files'
);
assert.ok(
  /app\.js\?v=1-3-60-live-home/.test(liveHtml) &&
    /fire-s-env\.js\?v=1-3-60-ver/.test(liveHtml) &&
