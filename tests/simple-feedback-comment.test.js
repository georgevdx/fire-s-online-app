...
assert.ok(
  /Version 1\.3\.115-toets/.test(stagingHtml) &&
    ...
    /TOETS-BLAD 1\.3\.115-toets/.test(read('staging/fire-s-env.js')) &&
    /1\.3\.115-toets/.test(read('staging/fire-s-env.js')) &&
    /fire-s-108-77-toets-clean/.test(read('staging/service-worker.js')),
  'Toets-blad must show 1.3.115-toets and drop the stuck 1.3.78 cache'
);
...
assert.ok(
  /Version 1\.3\.67/.test(liveHtml) &&
    ...
    /fire-s-108-79-live-sig/.test(read('service-worker.js')) &&
    /appVersion: staging \? '1\.3\.27-toets' : '1\.3\.67'/.test(read('fire-s-env.js')),
  'Live must sit the comment form and show 1.3.67'
);
