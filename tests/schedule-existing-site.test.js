...
assert.ok(/1\.3\.109-toets/.test(stagingEnv), 'Toets-blad version must be 1.3.109-toets');
assert.ok(
...
assert.ok(
  /id="appVersion" class="brand-version">Version 1\.3\.109-toets</.test(stagingHtml) &&
    /id="cloudVersion">1\.3\.109-toets</.test(stagingHtml),
  'Toets-blad must show Version 1.3.109-toets in the header without waiting for a script'
);
