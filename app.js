  ...
  try {
    ...
    if (profileError) {
      ...
      currentUserProfile = {
        ...
        role: 'guest',
        companyId: null,
        companyName: 'Access profile unavailable'
      };

      currentCompanyAccess = {
        status: 'unknown',
        plan: 'development',
        source: 'profile_load_failed'
      };
    ...
    currentUserProfile = {
      ...
      role: membership?.role || profile.role || 'guest',
...
// =====================================================
// FIRE-S ROLE VISIBILITY STABILITY PATCH - 14 JULY 2026
// One conservative role controller for the Home Command Centre.
// Unknown/loading access is never treated as Inspector.
// =====================================================
function normalizeFireSRole(roleValue) {
...
function clearLegacyHomeRoleLocks() {
  ...
  [
    ...
    'fire-s-home-rendering-121j',
    'fire-s-home-resolved-inspector-121j',
    'fire-s-home-resolved-management-121j'
  ].forEach(className => document.body.classList.remove(className));

  document.querySelectorAll(
    '.fire-s-inspector-hidden-card-121c, .fire-s-inspector-hidden-card-121d, .fire-s-inspector-hidden-card-121e'
  ).forEach(element => {
    element.classList.remove(
...
function setHomeCommandCardVisible(id, visible) {
...
function applyHomeRoleVisibility() {
  clearLegacyHomeRoleLocks();

  const role = normalizeFireSRole(currentUserProfile?.role);
  const isInspectorRole = role === 'inspector';
  const isManagerRole = role === 'manager' || role === 'viewer';
  const isOwnerRole = role === 'company_owner';
  const isSuperRole = role === 'super_admin';
  const isKnownRole = isInspectorRole || isManagerRole || isOwnerRole || isSuperRole;

  // IMPORTANT: while the Supabase profile/membership is still loading or
  // temporarily unavailable, do not hide any organisational level. The previous
  // patch treated this state as guest and made the navigation disappear.
  if (!isKnownRole) {
    [
      'cmdInspectionsBtn',
      'cmdScheduleBtn',
      ...
      'cmdCompanyBtn',
      'cmdServicesBtn'
    ].forEach(id => setHomeCommandCardVisible(id, true));

    if (document.body) {
      document.body.dataset.fireSResolvedRole = 'loading';
      document.body.classList.add('fire-s-role-management');
    }
    return;
  }

  setHomeCommandCardVisible('cmdInspectionsBtn', true);
  setHomeCommandCardVisible('cmdScheduleBtn', role !== 'viewer');

  // Executive/management cards.
  setHomeCommandCardVisible('cmdDashboardBtn', !isInspectorRole);
  setHomeCommandCardVisible('cmdFindingsBtn', !isInspectorRole);
  setHomeCommandCardVisible('cmdOverdueBtn', !isInspectorRole);

  // Reports is available to all authenticated organisational roles.
  setHomeCommandCardVisible('cmdReportsBtn', true);

  // Company administration is limited to owner and super admin.
  setHomeCommandCardVisible('cmdCompanyBtn', isOwnerRole || isSuperRole);

  // Services/support remains available to organisational roles.
  setHomeCommandCardVisible('cmdServicesBtn', true);

  const executiveHero = document.getElementById('complianceHeroCard');
  const executiveStats = document.querySelector('#mainCommandCentre .main-command-stats');
  const showExecutive = !isInspectorRole;
  if (executiveHero) executiveHero.style.setProperty('display', showExecutive ? '' : 'none', 'important');
  if (executiveStats) executiveStats.style.setProperty('display', showExecutive ? 'grid' : 'none', 'important');

  if (document.body) {
    document.body.dataset.fireSResolvedRole = role;
    document.body.classList.add(isInspectorRole ? 'fire-s-role-inspector' : 'fire-s-role-management');
  }
}
...
function hideReportsCommandCardSafe() {
  // Role stability patch: Reports visibility is controlled by applyHomeRoleVisibility().
  // Do not hide the card globally.
  return;
}

function openReportsCommandTemporarilyRemoved() {
  // Retained only as historical code. The working openReportsCommand above remains active.
  if (typeof showMainCommandMessage === 'function') {
    showMainCommandMessage('Reports Centre is available from the Reports card.');
  }
}
...
/* FIRE-S RC 1.2.2 - COMMAND CENTRE INITIAL COUNTS
   Fixes KPI cards showing 0 on first load and only correcting after
   opening the Projects page and returning Home.
*/
(function () {
  ...
  function refreshCommandCentreCounts(force = false) {
    if (!force && !homeIsVisible()) return;
    ...
    try {
      ...
      if (typeof renderComplianceCommandCentre === 'function') {
        renderComplianceCommandCentre();
      }
      ...
      console.warn('Command Centre initial count refresh failed:', error);
    }
  }

  function queueCommandCentreRefresh(delays = [0, 80, 250, 700]) {
    clearTimeout(refreshTimer);

    delays.forEach(delay => {
      setTimeout(() => {
        refreshCommandCentreCounts(true);
      }, delay);
    });
  }

  const originalSetProjects =
    typeof setProjects === 'function'
      ? setProjects
      : null;

  if (originalSetProjects && !window.fireSOriginalSetProjects122) {
    window.fireSOriginalSetProjects122 = originalSetProjects;

    setProjects = function fireSSetProjectsWithHomeRefresh122(projects) {
      const result =
        window.fireSOriginalSetProjects122.apply(this, arguments);

      queueCommandCentreRefresh([0, 120, 450]);
  ...
  if (originalShowHome && !window.fireSOriginalShowHome122) {
    window.fireSOriginalShowHome122 = originalShowHome;

    showHome = function fireSShowHomeWithFreshCounts122() {
      const result =
        window.fireSOriginalShowHome122.apply(this, arguments);

      queueCommandCentreRefresh([0, 50, 180, 500]);

      return result;
    };
  }

  const originalRestoreCloudSession =
    typeof restoreCloudSession === 'function'
      ? restoreCloudSession
      : null;

  if (originalRestoreCloudSession && !window.fireSOriginalRestoreCloudSession122) {
    window.fireSOriginalRestoreCloudSession122 = originalRestoreCloudSession;

    restoreCloudSession =
      async function fireSRestoreCloudSessionWithFreshCounts122() {
        const result =
          await window.fireSOriginalRestoreCloudSession122.apply(this, arguments);

        queueCommandCentreRefresh([0, 150, 600]);

        return result;
      };
  }

  const originalSafeDownload =
    typeof safeDownloadNewerCloudInspections === 'function'
      ? safeDownloadNewerCloudInspections
      : null;

  if (originalSafeDownload && !window.fireSOriginalSafeDownload122) {
    window.fireSOriginalSafeDownload122 = originalSafeDownload;

    safeDownloadNewerCloudInspections =
      async function fireSSafeDownloadWithFreshCounts122() {
        const result =
          await window.fireSOriginalSafeDownload122.apply(this, arguments);

        queueCommandCentreRefresh([0, 150, 500]);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    queueCommandCentreRefresh([0, 100, 400, 1000, 2000]);
  });

  window.addEventListener('load', () => {
    queueCommandCentreRefresh([0, 150, 600, 1400]);
  });

  window.addEventListener('pageshow', () => {
    queueCommandCentreRefresh([0, 100, 350]);
  });

  window.addEventListener('storage', event => {
    if (event.key === 'fireyeProjects') {
      queueCommandCentreRefresh([0, 100, 350]);
    }
  });

  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && homeIsVisible()) {
      queueCommandCentreRefresh([0, 120, 400]);
    }
  });

  // Lightweight safety check: refresh only when the stored project data changed.
  setInterval(() => {
  ...
  window.FireSRefreshCommandCentreCounts =
    queueCommandCentreRefresh;
})();
